...
import com.devsuperior.dscommerce.dtos.CustomErrorDTO;
import com.devsuperior.dscommerce.dtos.ValidationErrorDTO;
...
public class ControllerExceptionHandler {
    ...
    public ResponseEntity<CustomErrorDTO> customName(DataIntegrityViolationException e, HttpServletRequest request) {
        HttpStatus status = HttpStatus.NOT_FOUND;
        CustomErrorDTO err = new CustomErrorDTO(Instant.now(), status.value(), e.getMessage(), request.getRequestURI());
        return ResponseEntity.status(status).body(err);
    }
